...
import { ACTS, EXAMINES, INITIAL_STATE, INITIAL_FLAGS, PIVOT_STILLS, ENDINGS, classifyEnding } from './content';
...
export default function BiddingEngine() {
  ...
  const onVideoEnded = useCallback(() => {
    ...
    // Mykonos: triggersFinalCluster — apply the mutation, run cluster
    // classifier on resulting state to pick A/B/C/D.
    if (def.triggersFinalCluster) {
      setState((prev) => {
        const close = Math.max(0, Math.min(100, prev.mercy + (def.mutation.mercy ?? 0)));
        const ask = def.mutation.mercerLocked ?? false;
        const picked = classifyEnding(close, ask);
        setEndingId(picked as EndingId);
        setPhase('ending');
        return { ...prev, mercy: close };
      });
      return;
    }
